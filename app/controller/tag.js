...
class HomeController extends Controller {
  ...
  async add() {
    ...
    if (!await service.tag.search(body)) return (ctx.body = { code: 1, data: false, message: '该类别已存在' });
    await service.tag.add(body);
  ...
  async update() {
    const { ctx, service } = this;
    const body = ctx.request.body;
    if (await service.tag.update(body)) return (ctx.body = { code: 0, data: true, message: '编辑标签类别成功' });
  }
  // 删除标签类别
  async del() {
    const { ctx, service } = this;
    const { id } = ctx.params;
    if (await service.tag.del(id)) return (ctx.body = { code: 0, data: true, message: '删除标签类别成功' });
  }
}
